...
Assert-PackagePath -Repository $repository -Path (Join-Path $repository 'dist/nested/augustus-windows')
...
foreach ($relative in @('dist/../../outside', 'dist-neighbor/augustus', 'dist/augustus-a/../../outside')) {
    $rejected = $false
