...
$stage = Join-Path $outputRoot "augustus-html"
$archive = Join-Path $outputRoot "augustus-html.zip"
...
if (-not $SkipBuild) {
    Invoke-Checked { cargo build --profile wasm-release --target wasm32-unknown-unknown --bin augustus -j12 }
}
...
Invoke-Checked {
    wasm-bindgen "target/wasm32-unknown-unknown/wasm-release/augustus.wasm" `
        --target web `
        --out-dir $stage `
        --out-name augustus `
        --no-typescript
